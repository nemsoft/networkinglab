...
public class ChatServer3{
    ...
    public boolean reply(ClientHandler3 sender, String message) {
        if(sender.replier!=null) {
            try {
                PrintWriter output = new PrintWriter(sender.replier.socket.getOutputStream(), true);
                output.println(sender.name+" -> "+sender.replier.name+": "+message);
                System.out.println(sender.name+" -> "+sender.replier.name+": "+message);
            } catch (IOException e) {}
        }
        return true;
    }
    public boolean message(ClientHandler3 sender, String receiver, String message) {
        for(ClientHandler3 c:clients) {
            if(c.name==null) break;
            if(c.name.equals(receiver)) {
                try {
                    PrintWriter output = new PrintWriter(c.socket.getOutputStream(), true);
                    output.println(sender.name+" -> "+receiver+": "+message);
                    System.out.println(sender.name+" -> "+receiver+": "+message);
                    sender.replier = c;
                    c.replier = sender;
                    return true;
                } catch (IOException e) {
    ...
    public boolean nameTaken(String name) {
        for(ClientHandler3 c:clients) {
            if(c.name!=null&&c.name.equals(name)) return true;
        }
        return false;
    }
